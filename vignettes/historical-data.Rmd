---
title: "Working with historical raw data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working with historical raw data}
...
The Digitraffic API stores raw vehicle-level observations as daily CSV files
for every LAM station. Each row represents a single vehicle passing over the
measurement loop, capturing its speed, length, lane, direction, and vehicle
class — down to the centisecond.

The `dt_history_raw()` function downloads and parses these files into tidy
tibbles, enriched with a proper `datetime` column and human-readable vehicle
class labels.
...
Before fetching history, you need the station's `tms_number` (not the `id`!).
These two identifiers are different:

- **`id`** — used by `dt_station_data()` and `dt_station()` for real-time data
- **`tms_number`** — used by `dt_history_raw()` for historical CSV files
...
available via `dt_vehicle_classes()`:
...
The `vehicle_class_label` column in `dt_history_raw()` output already joins
these labels for you.
...
The `interval_ms` column records the time gap between consecutive vehicles
in the same lane. This is useful for studying congestion and following
distances:
...
To analyse multiple days, loop over dates with `lapply()` or `purrr::map()`:
...
Each day's CSV file is a separate API request, typically 1-5 MB. For large
date ranges:

- Consider spacing requests to stay under the 60 req/min throttle
- Store intermediate results locally with `saveRDS()` / `readRDS()`
...
A common workflow is to look up station details, then fetch historical data:

```{r metadata-plus-history}
# 1. Find a station
...
- [Digitraffic road traffic documentation](https://www.digitraffic.fi/tieliikenne/) (Finnish/English)
- [Digitraffic LAM documentation](https://www.digitraffic.fi/tieliikenne/lam/) — detailed sensor descriptions
- [API Swagger documentation](https://tie.digitraffic.fi/swagger/) — full endpoint reference
- [Getting started vignette](digitraffic.html) — real-time data, sensors, and station metadata
- [Station filtering vignette](station-filtering.html) — filtering by road, municipality, province, and bounding box
